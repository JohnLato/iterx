...
{-# INLINE folding #-}
folding :: (Monad m) => (b -> a -> b) -> b -> FoldM m a b
folding f s0 = foldingM (\b a -> return $! f b a) s0

{-# INLINE [1] foldingM #-}
foldingM :: (Monad m) => (b -> a -> m b) -> b -> FoldM m a b
foldingM  f s0 = FoldM f s0 return

{-# INLINE sums #-}
sums :: (Num a, Monad m) => FoldM m a a
sums = folding (+) 0

{-# INLINE products #-}
products :: (Num a, Monad m) => FoldM m a a
products = folding (*) 1

{-# INLINE count #-}
count :: (Num cnt, Monad m) => FoldM m a cnt
count = folding (\b _ -> b+1) 0

{-# INLINE foldLast #-}
foldLast :: (Monad m) => i -> FoldM m i i
foldLast x0 = folding (\_ x -> x) x0

{-# INLINE foldFirst #-}
foldFirst :: (Monad m) => FoldM m i (Maybe i)
...
{-# INLINE foldConst #-}
foldConst :: (Monad m) => a -> FoldM m i a
...
{-# INLINE toList #-}
toList :: (Monad m) => FoldM m a [a]
toList = FoldM loop id (return . ($ []))
  where
    {-# INLINE [0] loop #-}
